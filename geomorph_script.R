...
#phylogenetics ANOVAs for microhabitat, feeding biology, & phragmosis
...
plotGMPhyloMorphoSpace(tree_sorted,Y.gpa$coords, tip.labels = TRUE, node.labels = FALSE, ancStates = FALSE, xaxis=1, yaxis=2,
...
plotGMPhyloMorphoSpace(tree_sorted,Y.gpa$coords, tip.labels = TRUE, node.labels = FALSE, ancStates = FALSE, xaxis=3, yaxis=2,
...
legend("topleft", legend= levels(classifier$Hyperossified), pch=c(19,15), col = c("white","black"), 
       bty = "n", cex = 1)
text(logCS,regscore, labels = rownames(classifier), cex=0.25, pos = 4)
...
node_states<-freeK_RJ$data$anc_state_1[158:313]
pp<-freeK_RJ$data$anc_state_1_pp[158:313]
...
mycol2[diet=="invertebrate_predator"]<-"grey33"
